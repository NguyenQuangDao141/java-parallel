...
public final class ReciprocalArraySum {
    ...
    protected static double parManyTaskArraySum(final double[] input,
        ...
        long startTime = System.nanoTime();
        double sum = 0;
        List<ReciprocalArraySumTask> listTask = new ArrayList<>();
        ...
        ForkJoinTask.invokeAll(listTask);
        for (ReciprocalArraySumTask task:listTask
             ) {
            sum+=task.getValue();
        }
        long timeNano = System.nanoTime() - startTime;
        printResult("parManyTaskArraySum",timeNano,sum);
        return sum;
    }
    private static void printResult(String name, long nanoTime, double sum){
        System.out.printf(" %s complete in %8.3f millisecond, with sum = %8.5f \n",name,nanoTime / 1e6,sum);
    }
}
