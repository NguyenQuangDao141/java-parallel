    ...
    public static void runParallelFuzzyBarrier(final int iterations,
        ...
        for (int ii=0;ii<tasks;ii++){
            ...
            threads[ii]=new Thread(()->{
                ...
                if (right>n) right=n;
                for (int iter=0;iter<iterations;iter++){
                    threadPrivateMyNew[right]=(threadPrivateMyVal[right-1]+threadPrivateMyVal[right+1])/2;
                    threadPrivateMyNew[left]=(threadPrivateMyVal[left-1]+threadPrivateMyVal[left+1])/2;
                    int currentPhase = ph.arrive();
                    for (int j=left+1; j<right;j++){
                        threadPrivateMyNew[j]=(threadPrivateMyVal[j-1]+threadPrivateMyVal[j+1])/2;
                    }
